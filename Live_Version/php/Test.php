    		 ...
    		 while($innerResult = $emailResult->fetch_assoc()){
    			...
    			$lastEmailUnix = strtotime($lastEmail);
    		 	...
    		 	//{
					...
					$sqlAddress = "SELECT email FROM EMAIL_LIST";
					$addressResult = $conn->query($sqlAddress);
					$emailArray = array(); 
					while($innerResult = $addressResult->fetch_assoc()){
						array_push($emailArray, '<'.$innerResult['email'].'>');
					}
					
    		 	    notification($uid, $DOUBLEtemp, $emailArray);
