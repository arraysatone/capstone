  ...
  //Print data for current temp
  	if ($uidResult->num_rows > 0) {
	    ...
	    while($row = $uidResult->fetch_assoc()) {
		    $id = $row['id'];
		    $uid = $row['uid'];
	    	$cabNameSQL = "SELECT cabinetName FROM LOCATION_SENSORS WHERE uid = '".$uid."'";
	    	$cabNameResult = $conn->query($cabNameSQL);
	    	if ($cabNameResult->num_rows > 0) {
	    		while($innerRow = $cabNameResult->fetch_assoc()) {
	    			$cabName = $innerRow['cabinetName'];

	    			echo '<button id="'.$uid.'" type="button" class="newBtn newBtn-sq-nm bdrWhite" onclick="btnClick(this.id)">
			            <div class="newBtnTitle">
			            <span class="newBtnTitleText">'.$cabName.'</span><span><i class="newBtnTitleFav colorHidden fas fa-star"></i></span>
			            </div>
			            <hr>
